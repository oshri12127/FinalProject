$(function() {
    ...
    const multiplayer = db.ref('multiplayer');
    //const playersRef = multiplayer.child('players'); // Reference entire players folder 
    //const p1Ref = playersRef.child('p1'); // Reference entire P1 folder
    //const p2Ref = playersRef.child('p2'); // Reference entire P2 folder
    const winsRef = multiplayer.child('win');    // Reference both player losses
    const losesRef = multiplayer.child('losses');    // Reference both player wins
    const turnRef = multiplayer.child('turn'); // to track the turns
    const connectionsRef = multiplayer.child("connections"); // Folder to store each connection
    const connectedRef = db.ref(".info/connected");// Firebase's default Ref to track connections (boolean)
    const chatRef = multiplayer.child('chat'); // Reference chat
    ...
    const playerName = () => {
        ...
        connectionsRef.on('value', (snap) => { // If I just moved someone to my connection folder
            ...
            //pNameVal = firebase.auth().currentUser;
